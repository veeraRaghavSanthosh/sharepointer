...
module.exports = function(client) {
 ...
 console.log('list Items URL SP ' + host);
        ...
        host = "/sites/Training2/TestITCARApprovals/_api/web/lists/getByTitle('Tasks')/Items?$filter=((Status%20eq%20'Not Started')%20and%20(AssignedToId%20eq%20'42'))%20or%20((Status%20eq%20'Not Started')%20and%20(AssignedToId%20eq%20'44'))&$select=AssignedToId"
      ...
      byteStream: function(floderName, filename, cb) {
        var host = client.BASE_LIST_URL.substring(0,client.BASE_LIST_URL.indexOf('_api/'))+ "/_api/web/GetFolderByServerRelativeUrl('" + floderName + "')/Files('" + filename + "')/$value";
        return doRequest(host, function(err, items) {
          if (err) {
            return cb(err);
          }
      ...
      read: function(listId, itemId, cb) {
        ...
        // and / or selectFields  and / or expandFields to host 
        host = urlBuilder.augmentURL(host, client.fieldValuesAsText, null, client.selectFields, client.expandFields, client.orOperator, client.andOrOperator);
