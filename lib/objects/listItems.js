  ...
  var doRequest = require('../util/doRequest')(client),
    ...
    crudl = {
      ...
      byteStream: function(floderName, filename, cb) {
        var host = "/_api/web/GetFolderByServerRelativeUrl('CAR%20Documents')/Files('Document7.docx')/$value";

        console.log("in the sharepoint npm byteStream function");
      ...
      currentUser: function(cb) {
        var host = "/_api/web/CurrentUser?$filter=Title";
        console.log("logging host -> "+host);
      ...
      read: function(listId, itemId, cb) {
        ...
        if (client.filterFields.hasOwnProperty(length)) {
          ...
          if (client.filterFields.length === 1) {
            ...
            console.log("or operator logging "+client.orOperator);
            for (i = 0; i < client.filterFields.length; i++) {
              if (i === 0) {
                filterString = '(' + client.filterFields[0]['field'] + '%20eq%20' + "'" + client.filterFields[0]['value'] + "')";
              } else if (client.orOperator) {
                console.log("client response"+client.orOperator);
                filterString += '%20or%20(' + client.filterFields[0]['field'] + '%20eq%20' + "'" + client.filterFields[0]['value'] + "')";
              } else {
